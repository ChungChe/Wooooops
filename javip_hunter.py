...
class javip_hunter:
    ...
    def get_rapid_link_by_url_id(self, url_id):
        if url_id == None:
            return
        try:
            c = "select rapid_link from upjav_table where url_id LIKE '{}'".format(url_id)
            #self.__cur.execute('select url_id, rapid_link from upjav_table where pid=:URL_ID COLLATE NOCASE', {'URL_ID': url_id})
            self.__cur.execute(c)
            return  self.__cur.fetchall()
        except Exception as e:
            print("Exception in get_rapid_link {}, {}".format(url_id, e))
    def query_data(self, condition):
        fields = "post_date, url_id, title, actress, cover_link, preview_link, rapid_link, available"
        ...
        names = ['post_date', 'url_id', 'title', 'actress', 'cover_link', 'preview_link' 'rapid_link']
        dict_list = []
        for e in match:
            is_avial = False
            ...
            dict_list.append({'post_date': e[0], 'url_id': e[1], 'title': e[2], 'actress': e[3], 'cover_link': e[4], 'preview_link': e[5], 'rapid_link': e[6], 'available': is_avial})
        return dict_list 
    ...
    def update_post_date(self):
        ...
        match = self.__cur.fetchall()
        print("total {} records".format(len(match)))
        for m in match:
            toks = m[1].split()
            new_date = None
            if len(toks) == 1:
                ...
                # 1th April 2017
                day = toks[0].replace('st', '').replace('nd', '').replace('th', '')
                all_m = ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december']
                month = all_m.index(toks[1].lower()) + 1
                year = toks[2]
                new_date = '{}-{}-{}'.format(year, '{:02d}'.format(month), day.zfill(2))
            try:
    ...
    def get_post_info_bottom(self, soup):
        return soup.find('div', {'class': 'post-info-bottom'})
    def get_post_date(self, soup):
        pdate = soup.find('span', {'class': 'post-info-date'})
        cont = None
        if pdate != None:
            cont = pdate.contents[-1].replace('\n','').replace('\t','').replace('on','')
        else:
            old_pdate = soup.find('span', {'class': 'p_date'})
            cont = old_pdate.contents[0]

        return cont
    def is_censored(self, soup, soup2):
        jav_cat_secs = None
        if soup != None:
            jav_cat_secs = soup.findAll('a', {'rel': 'category tag'})
            for jav_cat_sec in jav_cat_secs:
        ...
        if soup2 != None:
            jav_cat_secs = soup2.findAll('a', {'rel': 'category tag'})
            for jav_cat_sec in jav_cat_secs:
                jav_cat = jav_cat_sec.contents[0]
                if jav_cat == "JAV Uncensored":
                    return False
    ...
    def scan_top_level(self, link):
        ...
        count = 0
        for page_num in range(1958, max_page_num + 1):
            ...
            for post in posts:
                ...
                if title == None: 
                    continue
                tit = title_link.split("http://{}".format(self.__site_name))
                url_id = title_link.split("http://{}/".format(self.__site_name))[1][:-1]
                ...
                post_info = inner_soup.find('div', {'class': 'post_info'})
                post_info_bottom = self.get_post_info_bottom(inner_soup)
                is_censored = self.is_censored(post_info_bottom, post_info)
                post_date = self.get_post_date(inner_soup)
                if post_date != None:
